...
while {true} do {
	...
	_btacp1 = if (isNil "B_tac_fac1") then {false} else {true};
	_btacp2 = if (isNil "B_tac_fac2") then {false} else {true};
	...
	_rtacp1 = if (isNil "R_tac_fac1") then {false} else {true};
	_rtacp2 = if (isNil "R_tac_fac2") then {false} else {true};
	...
	_gtacp1 = if (isNil "G_tac_fac1") then {false} else {true};
	_gtacp2 = if (isNil "G_tac_fac2") then {false} else {true};
...
// Ping logi and PL 
	if (_reinforcementsTotalCount > 0 ) then {
		...
		if (_btacp1 == true) then { format ["%1", _deploymentHint] remoteExec ["hint", B_tac_fac1]; };
		if (_btacp2 == true) then { format ["%1", _deploymentHint] remoteExec ["hint", B_tac_fac2]; };
		...
		if (_rtacp1 == true) then { format ["%1", _deploymentHint] remoteExec ["hint", R_tac_fac1]; };
		if (_rtacp2 == true) then { format ["%1", _deploymentHint] remoteExec ["hint", R_tac_fac2]; };
		...
		if (_gtacp1 == true) then { format ["%1", _deploymentHint] remoteExec ["hint", G_tac_fac1]; };
		if (_gtacp2 == true) then { format ["%1", _deploymentHint] remoteExec ["hint", G_tac_fac2]; };
	};
	sleep 30;
};
